...
Route any path to its rung/organ/record home:
```bash
...
```
docs/
├── AGENTS.md             ← doc-tree router
├── INDEX.md              ← GENERATED by `arch.py emit` (never hand-edited)
...
1b. **Doc-governance gates** — Run `python3 scripts/arch.py validate` after every change to `docs/`, `_org.yaml`, or a record. It must exit 0 (gates DG1–DG10: record schema, status enum, numbering, authority uniqueness, superseded vocabulary, historical quarantine, ownership, reference resolution, ledger integrity, canon↔code).
   ...
   - **Before answering "what does this word mean" or "who owns this?"** read `docs/foundations/44`; before answering "what did we decide / what broke?" read `docs/system/core/{decisions,regressions}/`.
