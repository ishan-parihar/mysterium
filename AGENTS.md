...
### 4.2 Current state: Phases 1–15 are ratified and built. **Phase 16 remains IN PROGRESS** (The Diagnostic Sweep — repair the instrument, then calibrate): **d2 BUILT** — `driveFixation` was a starved input, not a pinned observable; both production prose and the attributed fixture-only declaration channel now deliver it, F-6 is closed, and G42 locks drive, direction, accumulation, and attribution. **d3 BUILT 2026-09-25** — the historical 1/432 Emotional baseline came from the static reproducibility hash acting as policy; the first developmental offer now reserves the eligible line with the oldest positive theta timestamp, may cross a priority band, changes no candidate `priority`, and leaves the ranked tail unchanged. Production-path tests plus **G43** lock the contract; G43 counts finalized developmental provenance only, requires all eight canonical lines, and retains the 2% quietest/busiest floor. d1/d4/d5/d6/d7 are **all delivered as of 2026-09-25** and Phase 16 stays active on its OPEN items. **d1** — `memoryPage` gained a real producer measured in the envelope (block lines/chars plus the post-Veil surviving lines), while `renderBudget` was **reclassified rather than measured**: the direct path Veil-filters inside
`LLMClient` but the WebUI BFF path returns *before* the filter, so the filtered prompt is knowable on
one path and unknowable on the other; the orphan field and both write sites were deleted, not left dormant. `engagementRegisterHits` stays deliberately unmeasured. **d4** — candidate provenance reads the raw `candidateId` and separates `missing` from `unrecognised` (the residual 12.5 % is entirely `missing`). **d5** — a focused campaign pins one `Line:Stage` cell and **fails loudly** if any encounter it finalizes leaves that cell; the four injection seams (threshold mode, Holonic Return, curriculum interleave, training weave) are bypassed when a cell is pinned by BOTH axes, so `--line` alone stays ordinary play. **d6** — re-measured wide and focused; **no threshold moved**. **d7** — `docs/audits/CAMPAIGN-REPORT-2026-09-25.md`. Headline findings: all eight lines reached on G43's own roster (quietest/busiest 1/2) while the 12-campaign cohort leaves `Interpersonal` at 0 — two different populations, and the exclusion is **open**; wide-cohort entropy is `insufficient-data` by scale, and the focused pass produced the **first measurable cell** (`Cognitive:Red`, 12 compositions, entropy 2.138 against a 0.5 floor); the polarity loop is enterable but **inert** (70 readings, 0 reconciled, 100 % fallback). The 2026-09-24 campaign figures remain `provisional-simulated-cohort` baseline evidence; synthetic evidence can reject but never certifies. Kernel roster: **43 gates** (unchanged — d5/d6 added no gate). Built CLI, real-rater/real-play certification, deployment, and remote synchronization remain release/external prerequisites.
