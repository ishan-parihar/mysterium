# Emotional / Teal — Module Specification

## 1. Module Identity

- **Line:** Emotional
- **Stage:** Teal (Integral / Vision-Logic)
- **Energy Ray:** Indigo (6a)
- **Core Capacity:** Vision-logic affect — seeing emotional patterns across systems, reading group-level emotional waves, equanimity as emerging baseline, holding all emotional stages as valid while maintaining sovereign emotional centre.
- **Below (Green):** Pluralistic empathy — multi-perspective affect, co-regulation, empathy to out-groups.
- **Above (Turquoise):** Equanimity — affect is luminous, not gripping. Compassion without effort.

## 2. Shadow Taxonomy

| Quadrant | Name | Pattern |
|----------|------|---------|
| Dark-Addiction | The Emotional Cartographer | Addicted to mapping emotional patterns; cannot stop analysing group affect; uses emotional meta-awareness to avoid direct feeling |
| Dark-Allergy | The Empathic Pluralist | Refuses integral affect; stays in Green empathic overwhelm/sensitivity; "all feelings are equal" as defence against hierarchy |
| Golden-Addiction | The Premature Equanimist | Performs Turquoise equanimity without genuine integral emotional mastery; claims "I am beyond emotion" while unable to read group-level affect |
| Golden-Allergy | The Integral Affect Fortress | Integral emotional capacity present but refuses equanimous opening; terrified of "losing feeling"; maps all emotions but will not transcend them |

## 3. Compound Shadows

| # | Name | Cross-Line | Mechanism |
|---|------|-----------|-----------|
| 1 | The Affect Mapper | Emotional DA × Cognitive DA | Emotional cartography + meta-mapping = analyses everything, feels nothing |
| 2 | The Sensitive Holdout | Emotional DAll × Interpersonal DAll | Refuses integral affect + refuses transpersonal relating = Green-level sensitivity only |
| 3 | The Equanimity Performer | Emotional GA × Spiritual GA | Premature equanimity + premature non-dual = performed peace |
| 4 | The Feeling Fortress | Emotional GAll × Intrapersonal GAll | Integral affect fortress + integral self fortress = feels deeply but won't release |

## 4. Drive Dynamics

| Drive | Healthy Expression | Pathological Expression |
|-------|-------------------|----------------------|
| Eros | Reaches toward equanimous opening; vertical pull toward Turquoise-level luminous affect | Bypasses integral mastery; premature transcendence (GA) |
| Agape | Returns to hold Green-level sensitivity with compassion; integrates lower emotional stages | Collapses back into Green overwhelm; refuses hierarchy (DAll) |
| Agency | Maintains sovereign emotional centre amid group-level waves; acts from integral affect | Fortifies integral position; refuses release (GAll) |
| Communion | Reads and attunes to group-level emotional fields; cross-system affect resonance | Dissolves into mapping; loses self in analysis (DA) |

## 5. Dual Vectors

- **Heal/Evolve (Agape + Agency):** Integrates dark shadows — returns to direct feeling (heals DA), claims integral hierarchy (heals DAll). Grounds vision-logic affect in embodied sovereignty.
- **Evolve/Heal (Eros + Communion):** Dissolves golden shadows — releases fortress walls (heals GAll), surrenders performed equanimity for genuine opening (heals GA). Reaches toward luminous affect through authentic group attunement.

## 6. Scoring Parameters

| Parameter | Value |
|-----------|-------|
| Half-life | 16 days |
| Max decay | 18% |
| Drive-health weight | 0.35 |
| Shadow-drag coefficient | 0.22 |
| Cross-line bonus | +0.15 |

## 7. Modality Map

| Modality | Unique Probe Axis |
|----------|------------------|
| Deterministic | Objective integral affect measurement — group-level pattern detection, equanimity metrics |
| Language-Reflective | Verbal articulation of integral emotional life — depth vs. performance in affect-talk |
| Scenario-Choice | Integral emotional dilemmas — holding all stages while committing to action |
| Embodied-Somatic | Body as site of integral affect — embodied equanimity, physical holding of group emotion |
| Strategic-Planning | Planning integral emotional life — structuring equanimity practice, cross-level management |
| Social-Cooperative | Shared integral affect — group attunement, cross-altitude emotional mentorship |
| Immersive-RPG | Ecological integral affect — spontaneous vision-logic emotion in free-play |
