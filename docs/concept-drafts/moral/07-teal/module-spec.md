# Moral / Teal — Module Spec

## 1. Module Identity

| Field | Value |
|---|---|
| Line | Moral |
| Stage | 7 — Teal / Integral |
| Ray | Indigo (6a) |
| Core capacity | Holonic ethics — protecting the most depth for the most span |
| Below | Green moral: contextual ethics, care-justice integration, moral pluralism |
| Above | Turquoise moral: kosmocentric all-sentient ethics, compassion without object |

## 2. Drive Model

| Drive | Healthy expression | Pathological expression |
|---|---|---|
| Eros | Reaches toward kosmocentric ethics; yearns for moral transcendence | Golden-Addiction: performs Turquoise compassion without integral mastery |
| Agape | Integrates Green contextual ethics into holonic framework; honours partial truths | Dark-Addiction: endlessly maps ethical hierarchies, avoids moral action |
| Agency | Executes holonic harm-trade-offs with precision and courage | Golden-Allergy: refuses kosmocentric opening, clings to integral structure |
| Communion | Mentors across developmental altitudes; shares integral moral reasoning | Dark-Allergy: refuses integral ethics, remains in Green pluralism |

## 3. Shadow Quadrants

| Quadrant | Name | Pattern |
|---|---|---|
| Dark-Addiction | The Ethical Architect | Addicted to holonic frameworks; maps moral hierarchies compulsively; uses integral ethics to avoid direct moral action |
| Dark-Allergy | The Contextual Pluralist | Refuses integral ethics; stays in Green contextual morality; 'no framework is better than another' |
| Golden-Addiction | The Premature Compassionist | Performs Turquoise kosmocentric ethics without genuine integral mastery; claims 'all is compassion' while unable to execute holonic trade-offs |
| Golden-Allergy | The Integral Ethics Fortress | Holonic ethics present but refuses kosmocentric opening; terrified of losing moral structure; ranks all ethics but will not transcend ranking |

## 4. Compound Shadows

| # | Name | Cross-line | Pattern |
|---|---|---|---|
| 1 | The Ethical Mapper | Cognitive DA | Ethical architecture + meta-mapping = maps all morality, acts on none |
| 2 | The Contextual Holdout | Interpersonal DAll | Refuses integral ethics + refuses integral relating = Green-level morality only |
| 3 | The Compassion Performer | Emotional GA | Premature kosmocentric + premature equanimity = performed universal love |
| 4 | The Holonic Ethics Fortress | Willpower GAll | Integral ethics fortress + integral will fortress = structures everything, releases nothing |

## 5. Scoring Parameters

| Parameter | Value |
|---|---|
| Half-life | 18 days |
| Max decay | 15% |
| Drive-health weight | 0.30 |
| Shadow-drag | 0.22 |
| Cross-line bonus | +0.15 |

## 6. Developmental Vectors

| Vector | Direction | Mechanism |
|---|---|---|
| Heal/Evolve | Bottom-up | Agape + Agency integrate dark shadows — the Ethical Architect must act, the Contextual Pluralist must rank |
| Evolve/Heal | Top-down | Eros + Communion dissolve golden shadows — the Premature Compassionist must master holonics, the Fortress must release structure |

## 7. Modality Coverage

| Modality | Unique probe axis |
|---|---|
| Deterministic | Objective holonic ethics measurement — depth-span trade-off accuracy |
| Language-Reflective | Verbal articulation of integral moral reasoning — depth vs. performance |
| Scenario-Choice | Integral moral dilemmas requiring holonic prioritisation |
| Embodied-Somatic | Body as site of integral ethics — embodied moral intuition |
| Strategic-Planning | Planning integral moral life — structuring holonic commitments across time |
| Social-Cooperative | Shared integral ethics — group holonic deliberation, cross-altitude mentorship |
| Immersive-RPG | Ecological integral ethics — spontaneous holonic reasoning in free-play |
