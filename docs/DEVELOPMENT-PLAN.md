...
`docs/audits/COUNCIL-ORTHOGONALITY-AUDIT-2026-09-24.md` §4–§6). The `AgentRole → CouncilRole`
binding lands in the delegation path with the **standing block** ([MY MANDATE]/[MY VIEW]/
[MY BOUNDARIES]/[MY TOOLS]/[MY SESSION]) plus the access tools (`read_my_scope`, `read_band`,
`read_session_log`, `analyze_session_logs`, `request_envelope_review`) with per-band authorization
and a recorded refusal. G32 extended: an out-of-binding band fails closed; the standing block must
be complete for every deployed role.
12. **d12 — Build the dispatcher** (audit O10 — the largest operational gap: the council is a
fully-built workforce with no summoner in play). The trigger table (audit §3) becomes strategy:
`schedulePresence` gains its live-loop caller, deterministic per `43 §3.3` (same state ⇒ same role
summoned), with the transformation moment (`17`) reserved for the whole-council summon and crisis
bypassing strategy (`43 §4.7`).
