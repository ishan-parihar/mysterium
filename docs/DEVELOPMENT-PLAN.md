...
- **Documentation + KB integrity** — `_org.yaml → pending`: `RT-ARCHIVE-MIGRATIONS`,
  `KB-SKILLS-PROVENANCE` and `KB-FOUNDATIONS-INGEST`. Closed in the 2026-09-20 conformance pass:
  gate fixtures (`RT-GATE-FIXTURES`), corpus reconciliation (`RT-CORPUS-RECONCILE`, gate `DG21`),
  orphan-script triage (`KB-ORPHAN-TRIAGE`, gate `DG20`), and `validate --json` +
  the per-pass index (`KB-VALIDATE-JSON`).
