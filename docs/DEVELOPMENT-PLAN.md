...
4. **d4 — The calibration pass** (planned read-only script, `@script-status: probe` — exits
   ...
   **5 cells with 0 measurable** (fewer than 8 compositions each, so the entropy verdict is
   ...
   question), and **`thetaStaleness` is 0.000 on every line**, a second pinned observable alongside
   `driveFixation`. Encounter traffic also concentrates: Cognitive 26 / Intrapersonal 24 / Spiritual 24
