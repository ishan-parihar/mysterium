...
| 1 | `cli-game.ts` split + calibration extraction to `src/core/` | > 1 000 | **PARTIAL 2026-09-24** — the calibration block landed (Phase 14 **d3**, `InitialAltitudeInference` + `QuickCalibrationScoring`, commit `e5e536f`). **Stages A · B · C of the split landed too**: `scripts/cli/` now holds `config.ts` · `data.ts` · `render.ts` (the pure helpers — 613 lines out of the runner, and `tests/cli/RenderHelpers.test.ts` is the first unit test the CLI has ever had), `flags.ts` (131 lines, the invocation state and its derivation) and `output.ts` (342 lines, the flag-reading printers). The runner is down to **5 199** lines from 5 770. **Remaining stage D below** |
...
| 4 | `AgenticOrchestrator` collaborator extraction | > 1 000 | **IN PROGRESS 2026-09-24** — collaborator 1/… extracted: `assessments/shadowSignals.ts` (the two shadow-keyword detectors + the precedence order they encode, out of the class as functions of their argument) with `tests/core/assessments/ShadowSignals.test.ts` as its lock. collaborators 1–3 done — `shadowSignals.ts` · `promptBlocks.ts` (the journey blocks) · `fallbackNarrative.ts` (the degradation narrative, with its two behavioural properties locked: the player's words are never quoted back, and silence has its own band). Next: the task presenters (`selectTaskForModality`, `generateModalityFallbackTask`, `presentModuleTask`), which need the renderer surface passed in rather than imported |
