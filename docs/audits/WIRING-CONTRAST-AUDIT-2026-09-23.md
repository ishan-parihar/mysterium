...
| **d10** | **The Polarity Pool (W11 — the finding that makes personalization real). RESHAPED 2026-09-24 by user ratification** (originally "candidate multiplicity per cell"). The library holds one rendering per (cell, tier), so a cell-targeted pool returns a fixed five and the UDV's bands have nothing to order. The fix is now four layers: (1) a similarity/opposition index over the tag ontology's axis space *derives* renderings (dynamic count, per-cell floor — one familiar-capable, one unfamiliar-capable); (2) each encounter resolves a familiar/unfamiliar polarity decision, unfamiliar flavour facing the cell's active shadow with shadow-severity-scaled dosage (noveltyBudget's first consumer), aversion veto untouched; (3) a spiral resolution *reading* (not a binary verdict) is proposed by the local System-1 layer and ratified by the orchestrator — repeated confirmations reach `reconciled` (46 §4.3's falsifiable state), disapproval re-opens the pair + raises shadow severity, every reading profile-updates through the background workers, and a cell is never closed (orthogonal-dimension probe coverage gates profiling completion); (4) top-1 primary + pole named reaches the prompt, alternates hidden. Full spec: the plan's Phase 13 d10 entry. | `candidateLibrary`, `pooling.ts`, new `polarityIndex` + resolution modules, `dialecticEngine` states, Distortion Ledger | The Phase 13 differential criterion holds: swapping ONE band changes `context.pooled`; a disconfirming reading measurably raises the next encounter's unfamiliar dosage; the per-cell floor is asserted; gates G27/G33 still green |
