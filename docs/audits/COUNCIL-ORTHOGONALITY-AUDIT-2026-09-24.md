...
| 5. d10 | ✅ **Built 2026-09-24** — **the Polarity Pool** (reshaped from "candidate multiplicity" by user ratification: derived index, shadow-scaled polarity, spiral reading, System-1 layer). The multiplicity this row awaited is now derived from the axis-space index, and the per-cell floor + differential are kernel-gated (**G35**, kernel 34 → 35). Phase 13 is complete | the plan's Phase 13 d10 build record |
