...
### Carried forward (recorded, not fixed — outside the ratified d2 scope; F7/F9 since closed by Phase 15 d0 and marked below, F2/d4 and d5 closed by Phase 14 d4/d5)

| # | Finding | Disposition |
|---|---|---|
| **F7** | `PlayerResponse.writeInValue` / `questionText` are populated by the orchestrator but **dropped by `processOutcome`** — the fields exist, the doc-comment says `encounter-log.md` needs them, and nothing reads them from a record. | **CLOSED (Phase 15 d0; user ruling F-1: WIRE IT).** Both declared on `ConsequenceRecord` with the why-not-on-the-response doc-comment, carried by `processOutcome`, set on **both** orchestrator finalise paths, and read from the record by the CLI (`cr.writeInValue` / `cr.questionText` — replacing `outcome.playerWriteIn` plus a private orchestrator field behind `as any`). **The extraction found a second defect in the same field:** `_lastQuestionText` was assigned the composed `fullPrompt` (narrative intro glued to the question), so the field named `questionText` held an intro — and the log slices to 500 chars, so a long intro could push the actual question out of the evidence. It now holds the question; what is *asked* is still `fullPrompt`. The live smoke showed the log rendering `**Question:**` followed by a blank line (label detached from content), so `appendEncounterLog` now collapses whitespace per value — `tests/infra/EncounterLog.test.ts` locks that and the focus extractor's ability to read a question back out. |
| **F9** | The facet compiler's quadrant aliasing is unwired (above) — the §2 Shadow-Archetype heading normalisation does not run. | **CLOSED (Phase 15 d0): claim deleted, not re-authored.** The `QUADRANTS`/`QUADRANT_ALIASES` declaration was retired in d2a and replaced with a RETIRED note that names the step that does not happen — re-authoring it would encode a normalisation nothing performs, i.e. a claim the tree cannot corroborate. The §2 extraction keeps its **fail-closed** check (a module-spec whose shadow section does not yield all 4 quadrants exits 1), which is the enforcement that matters: it is what would catch a real heading-shape drift, and it runs on every compile rather than in a table nobody reads. |
